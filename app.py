...
.chart-container { display:flex; align-items:flex-end; gap:2px; height:180px; padding:10px 0; overflow-x:auto; }
.chart-col { display:flex; flex-direction:column; align-items:center; flex:1; min-width:18px; }
.chart-bar { background:var(--blue); border-radius:3px 3px 0 0; width:100%; min-height:1px; transition:height 0.3s; }
...
function showDeviceChart(device) {
    ...
    .then(function(result) {
        ...
        data.counts.forEach(function(d) {
            ...
            barsHtml += '<div class="chart-bar" style="height:' + pct + '%"></div>';
