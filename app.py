...
<div><h1>JamSlayer</h1><span class="header-sub">AGS1 Sort Center - PEC Blockage Trend Analysis</span></div>
</div>
...
<th>#</th>
<th class="sortable" data-col="device" data-type="str">Device</th>
<th class="sortable" data-col="plc" data-type="str">PLC</th>
...
<th class="sortable ratio-header" data-col="increase_ratio" data-type="num">Ratio <span class="ratio-hint">Recent avg / Early avg. Shows how much worse this device is now vs the start. 3.0x = tripled in frequency.</span></th>
...
function renderDashboard(data) {
    ...
    if (data.generated_at) {
        ...
        document.getElementById('last-updated').textContent = 'Updated: ' + dt.toLocaleDateString() + ' ' + dt.toLocaleTimeString();
    }
    ...
    if (data.top25 && data.top25.length > 0) {
        ...
        data.top25.forEach(function(d, i) {
            ...
            row += '<td class="plc-name">' + (d.plc || '') + '</td>';
...
function showDeviceChart(device) {
    ...
    .then(function(result) {
        ...
        data.counts.forEach(function(d) {
            ...
            barsHtml += '<div class="chart-bar" style="width:' + pct + '%"></div>';
